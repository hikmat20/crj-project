...
class Check_hscode extends Admin_Controller
{
	...
	public function getData()
	{
        ...
        FROM view_Check_hscode, (SELECT @row_number:=0) as temp WHERE 1=1 $where  
	...
	public function edit($id)
	{
		...
		$trucking 		= $this->db->get_where('Check_hscode', array('id' => $id))->row();
		$states 		= $this->db->order_by('name', 'ASC')->get_where('states', ['country_id' => '102'])->result_array();
		$cities 		= $this->db->order_by('name', 'ASC')->get_where('cities', ['country_id' => '102'])->result_array();
		$ArrStates 		= array_column($states, 'name', 'id');
		$ArrCities 		= [];
	...
	public function save()
	{
		...
		$data = $post;
		$data['id'] = isset($post['id']) && $post['id'] ? $post['id'] : $this->Check_hscode_model->generate_id();
		$detail = $post['detail'];
		unset($data['detail']);
	...
	public function delete()
	{
		...
		$trucking 	= $this->db->get_where('Check_hscode')->row_array();
		$data = [
			'status' => '0',
		...
		$this->db->trans_begin();
		$this->db->update('Check_hscode', $data, ['id' => $id]);
