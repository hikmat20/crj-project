...
<head>
    <title><?= $request->customer_name; ?> [<?= str_replace("/", "-", $request->number); ?>]</title>
    <style>
        body {
            width: 100%;
            font-family: Arial;
...
<body>
    ...
    <table width="100%" style="font-size: 7pt;">
        ...
        <tr>
            ...
            <td>:</td>
            <td><?= isset($request->last_checked_by) ? $request->last_checked_by : '-'; ?></td>
            <td></td>
            <th>Last Checked At</th>
            <td>:</td>
            <td><?= isset($request->last_checked_at) ? $request->last_checked_at : '-'; ?></td>
        </tr>
    </table>
    <!-- details -->
    <hr>
