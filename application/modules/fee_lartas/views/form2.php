    ...
    <div class="form-group row">
        ...
        <div class="col-md-9">
            <input type="text" readonly class="form-control" id="id" name="id"
                value="<?= (isset($fee)) ? $fee->id : null; ?>" placeholder="Auto">
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-md-9">
            <select required class="form-control select" id="customer_id" name="customer_id">
                ...
                <option value="<?= $cust->id_customer; ?>"
                    <?= (isset($fee->customer_id) && $fee->customer_id == $cust->id_customer) ? 'selected' : ''; ?>>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-9">
            <textarea type="text" class="form-control" id="description" name="description"
                placeholder="Description"><?= (isset($fee) && $fee->description) ? $fee->description : null; ?></textarea>
        </div>
    </div>
    <div class="form-group row">
        <div class="col-md-3">
                    ...
                    <tr>
                        <td class="tx-dark tx-bold"><?= $lts->name; ?>
                            <input type="hidden" name="detail[<?= $n; ?>][id]"
                                value="<?= (isset($ArrDtl[$lts->id]->id) && $ArrDtl[$lts->id]->id) ? $ArrDtl[$lts->id]->id : ''; ?>">
                            <input type="hidden" name="detail[<?= $n; ?>][lartas_id]"
                                value="<?= (isset($lts->id) &&  $lts->id) ?  $lts->id : ''; ?>">
                        </td>
                        <td>
                            <input type="text" name="detail[<?= $n; ?>][value]"
                                value="<?= (isset($ArrDtl[$lts->id]->cost_value) && $ArrDtl[$lts->id]->cost_value) ? number_format($ArrDtl[$lts->id]->cost_value) : '0'; ?>"
                                class="form-control number-format text-right border-top-0 border-right-0 border-left-0 rounded-0"
                                placeholder="0">
                        </td>
                        <td>
                            <select name="detail[<?= $n; ?>][unit]" class="form-control">
                                ...
                                <option value="<?= $k; ?>"
                                    <?= (isset($ArrDtl[$lts->id]->unit) && $ArrDtl[$lts->id]->unit == $k) ? 'selected' : ''; ?>>
                                    <?= $unit; ?></option>
                                <?php endforeach; ?>
