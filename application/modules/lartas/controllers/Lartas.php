...
class Lartas extends Admin_Controller
{
	//Permission
	protected $viewPermission 	= 'Lartas.View';
	protected $addPermission  	= 'Lartas.Add';
	protected $managePermission = 'Lartas.Manage';
	protected $deletePermission = 'Lartas.Delete';
	...
	public function __construct()
	{
		...
		$this->template->title('Manage Lartas');
	...
	public function getData()
	{
        ...
        FROM lartas, (SELECT @row_number:=0) as temp WHERE 1=1 $where  
	...
	public function edit($id)
	{
		...
		$lartas = $this->db->get_where('lartas', array('id' => $id))->row();
		$data = [
			'data' => $lartas
		];
	...
	public function save()
	{
		...
		$data 		= $post;
		$data['id'] = isset($post['id']) && $post['id'] ? $post['id'] : $this->Lartas_model->generate_id();

		$this->db->trans_begin();
		if (isset($post['id']) && $post['id']) {
			$data['modified_at']	= date('Y-m-d H:i:s');
			$data['modified_by']	= $this->auth->user_id();
			$this->db->where('id', $post['id'])->update("lartas", $data);
		} else {
			$data['created_at']		= $data['modified_at'] = date('Y-m-d H:i:s');
			$data['created_by']		= $data['modified_by'] = $this->auth->user_id();
			$this->db->insert("lartas", $data);
		}

		if ($this->db->trans_status() === FALSE) {
			$this->db->trans_rollback();
			$return	= array(
				'msg'		=> 'Failed save data Lartas.  Please try again.',
				'status'	=> 0
			);
			$keterangan     = "FAILD save data Lartas " . $data['id'] . ", Lartas name : " . $data['name'];
			$status         = 1;
			...
			$return	= array(
				'msg'		=> 'Success Save data Lartas.',
				'status'	=> 1
			);
			$keterangan     = "SUCCESS save data Lartas " . $data['id'] . ", Lartas name : " . $data['name'];
			$status         = 1;
			$nm_hak_akses   = $this->addPermission;
			$kode_universal = $data['id'];
			$jumlah         = 1;
