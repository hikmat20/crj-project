...
class Rate_model extends BF_Model
{
    ...
    function generate_id($kode='') {
      $query = $this->db->query("SELECT MAX(id_rate) as max_id FROM ms_rate_ferreous");
      $row = $query->row_array();
      $thn = date('y');
      $max_id = $row['max_id'];
      $max_id1 =(int) substr($max_id,3,5);
      $counter = $max_id1 +1;
      $idcust = "MR".$thn.str_pad($counter, 5, "0", STR_PAD_LEFT);
      return $idcust;
	}
	...
	public function getRate(){
		$this->db->select('a.*, b.type_rate as type_rate');
		$this->db->from('ms_rate_ferreous a');
		$this->db->join('ms_type_rate b','b.id_type_rate=a.id_type_rate');
		$this->db->where('a.deleted','0');
		$query = $this->db->get();		
		return $query->result();
	}
		public function getViewRate($id){
		$search = "a.deleted='0' and a.id_rate='$id'";	
		$this->db->select('a.*, b.type_rate as type_rate');
		$this->db->from('ms_rate_ferreous a');
		$this->db->join('ms_type_rate b','b.id_type_rate=a.id_type_rate');
		$this->db->where($search);
