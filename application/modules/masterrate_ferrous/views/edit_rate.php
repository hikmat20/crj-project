		...
		<form id="data-form" method="post">
					  <div class="col-sm-12">
						   <div class="input_fields_wrap2">
										...
										<div class="col-md-4">
									    <label for="customer">Cost Element</label>
									    </div>
										 <div class="col-md-6" hidden>
											<input type="text" class="form-control" id="" required name="id_rate" value="<?= $rate->id_rate ?>" placeholder="Nama Inventory">
									    </div>
									    <div class="col-md-6">
											<input type="text" class="form-control"  id="" required name="cost_element" value="<?= $rate->cost_element ?>" placeholder="Nama Inventory">
									    </div>
										</div>
										<div class="form-group row">
										<div class="col-md-4">
									    <label for="customer">Type Rate</label>
									    </div>
									    <div class="col-md-6">
											  <select id="id_type_rate"  name="id_type_rate"   class="form-control select" required>
						<option value="">-- Rate Type --</option>
						<?php foreach ($results['type'] as $type){
						$select = $rate->id_type_rate == $type->id_type_rate ? 'selected' : '';
						?>
						<option value="<?= $type->id_type_rate?>" <?= $select ?>><?= $type->type_rate?></option>
						<?php } ?>
					  </select>
									    </div>
										</div>
										<div class="form-group row">
										<div class="col-md-4">
									    <label for="customer">Persentase Rate</label>
									    </div>
									    <div class="col-md-4">
											<input type="text" class="form-control"  id="" required name="presentase_rate" value="<?= $rate->presentase_rate ?>" placeholder="Nama Inventory"> %
									    </div>
										</div>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'masterrate_ferrous/SaveEditRate';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
