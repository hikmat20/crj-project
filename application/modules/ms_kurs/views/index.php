...
<div class="box">
	...
	<!-- /.box-header -->
	<div class="box-body">
		<table id="example1" width="50%" class="table table-bordered table-striped">
		<thead>
		<tr>
			<th width="5">#</th>
			<th width="13%">Kurs Terbaru</th>
			<th width="13%">Tgl Dibuat</th>
			<th>Status</th>
		...
		<tr>
		    <td><?= $numb; ?></td>
			<td><?= $record->nilai_kurs ?></td>
			<td><?= $record->created_on ?></td>
			<td>
				<?php if($record->aktif == 'Y'){ ?>
...
<script type="text/javascript">
	...
	$(document).on('click', '.edit', function(e){
		...
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Edit Kurs</b>");
		$.ajax({
			type:'POST',
			url:siteurl+'ms_kurs/editKurs/'+id,
	...
	$(document).on('click', '.view', function(){
		...
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Tambah Kurs</b>");
		$.ajax({
			type:'POST',
			url:siteurl+'ms_kurs/addKurs',
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
