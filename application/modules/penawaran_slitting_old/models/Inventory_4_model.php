...
class Inventory_4_model extends BF_Model
{
    ...
    function generate_code($kode='') {
      $query = $this->db->query("SELECT MAX(id_slitting) as max_id FROM tr_penawaran_slitting");
      $row = $query->row_array();
      $thn = date('y');
      $max_id = $row['max_id'];
	...
	$tahun = date('Y');
		...
		$blnthn = date('Y-m');
      $query = $this->db->query("SELECT MAX(no_surat) as max_id FROM tr_penawaran_slitting WHERE month(tgl_penawaran)='$bulan' and Year(tgl_penawaran)='$tahun'");
      $row = $query->row_array();
      $thn = date('T');
      $max_id = $row['max_id'];
	...
	public function CariPenawaran(){
		...
		$this->db->from('tr_penawaran_slitting a');
		$this->db->join('master_customers b','b.id_customer=a.id_customer');
		$this->db->order_by('a.id_slitting', DESC);
		$query = $this->db->get();	
		return $query->result();
	}
