                            ...
                            <thead class="bg-light">
                                ...
                                <tr>
                                    <th class="text-center border border-top-0 border-right-0">
                                        Price (<span id="type-price-text"><?= ($header->price_type == 'FOB') ? 'FOB' : 'CFR/CIF'; ?></span>)</th>
                                    <th class="text-center align-middle">BM</th>
                                    <th class="text-center align-middle">PPH</th>
                                </tr>
                            </thead>
                            <tbody class="tx-dark">
                                <?php $n = $totalPrice = $totalBMPrice = $totalPPHPrice = $gtotalBMPrice = $gtotalPPHPrice = $totalNonLartas = 0;
                                $no_image = base_url('assets/no-image.jpg');
                                if ($details) foreach ($details as $dt) : $n++;
                                    $totalPrice      += $dt->price;

                                    $totalBMPrice     = $dt->price * ($ArrHscode[$dt->origin_hscode]->bm_e / 100);
                                    $totalPPHPrice    = ($dt->price + $totalBMPrice) * ($ArrHscode[$dt->origin_hscode]->pph_api / 100);
                                    $gtotalBMPrice   += $totalBMPrice;
                                    $gtotalPPHPrice  += $totalPPHPrice;
                                ...
                                ?>
                                    <tr class="tx-dark">
                                        ...
                                        <td class="text-right"><?= ($totalBMPrice) ? number_format($totalBMPrice, 2) : '0' ?>
                                            <input type="hidden" name="detail[<?= $n; ?>][total_bm]" value="<?= ($totalBMPrice) ? $totalBMPrice : '0'; ?>">
                                        </td>
                                        <td class="text-right"><?= ($totalPPHPrice) ? number_format($totalPPHPrice, 2)  : '0' ?>
                                            <input type="hidden" name="detail[<?= $n; ?>][total_pph]" value="<?= ($totalPPHPrice) ? $totalPPHPrice : '0'; ?>">
                                        </td>
                                ...
                                <tr class="bg-light">
                                    ...
                                    <th class="text-right tx-dark font-weight-bold"><?= number_format($gtotalBMPrice, 2); ?></th>
                                    <th class="text-right tx-dark font-weight-bold"><?= number_format(($gtotalPPHPrice) ?: '0', 2); ?></th>
                                    <th></th>
                                </tr>
                                <tr>
                    ...
                    <div class="col-md-7">
                        ...
                        <div id="loadLartas">
                            <table class="table table-sm table-borderless">
                                ...
                                <tbody>
                                    <?php $n = 0;
                                    if (isset($fee_lartas) && $fee_lartas) foreach ($fee_lartas as $lts) : $n++; ?>
                                        <tr>
