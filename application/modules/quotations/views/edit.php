                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle wd-lg-30p">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" value="<?= number_format($header->total_product, 2); ?>" class="bg-transparent form-control border-0 text-right bg-white tx-16-force tx-dark tx-bold" placeholder="0" readonly autocomplete="off" value="<?= number_format(($totalPrice) ?: '0', 2); ?>">
                                                </div>
                                            </td>
                                        </tr>
                                        <tr class="table-secondary">
                        ...
                        <table class="table table-sm table- border table-striped">
                            ...
                            <tfoot class="tx-dark">
                                <tr style="background-color:#ccc">
                                    ...
                                    <th class="text-right"><?= number_format($header->grand_total, 2); ?></th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '#percentage_dp1', function() {
            let id = ($(this).data('id'));
            ...
            let amount1 = parseFloat((total_product * percent) / 100)
            $('#amount_' + id).val(new Intl.NumberFormat().format(amount1.toFixed(2)))

            let amount2 = parseFloat(total_product - amount1)
            $('#amount_dp2').val(new Intl.NumberFormat().format(amount2.toFixed(2)))

            let amount3 = parseFloat($('#amount_dp3').val().replace(/\,/g, '') || 0)
            let amount4 = parseFloat(grand_total - (amount1 + amount2 + amount3))
            $('#amount_dp4').val(new Intl.NumberFormat().format(amount4.toFixed(2)))
        })

        $(document).on('change', '#percentage_dp3', function() {
            let id = ($(this).data('id'));
            let percent = parseInt($(this).val() || 0);
            let total_product = parseFloat($('#total_product').val().replace(/\,/g, '') || 0)
            let grand_total = parseFloat($('#grand_total').val().replace(/\,/g, '') || 0)
            let amount1 = parseFloat($('#amount_dp1').val().replace(/\,/g, '') || 0)
            let amount2 = parseFloat($('#amount_dp2').val().replace(/\,/g, '') || 0)

            let amount3 = parseFloat(total_product * percent) / 100
            $('#amount_' + id).val(new Intl.NumberFormat().format(amount3.toFixed(2)))
