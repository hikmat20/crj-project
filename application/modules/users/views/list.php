			...
			<tbody>
				<?php foreach ($results as $record) : ?>
					<tr>
						...
						<td><?= $record->city ?></td>
						<td><?= $record->phone ?></td>
						<td><?= ($record->status == 0) ? "<label class='label label-danger'>" . lang('users_td_aktif') . "</label>" : "<label class='label label-success'>" . lang('users_aktif') . "</label>" ?></td>
						<?php if ($ENABLE_MANAGE) : ?>
							<td style="padding-right:20px">
								<a class="text-black" href="<?= site_url('users/setting/edit/' . $record->id_user); ?>" data-toggle="tooltip" data-placement="left" title="Edit Data"><i class="fa fa-edit"></i></a>
								<?php if ($record->id_user != 1) : ?>
									&nbsp;|&nbsp;
									<a class="text-black" href="<?= site_url('users/setting/permission/' . $record->id_user); ?>" data-toggle="tooltip" data-placement="left" title="Edit Hak Akses"><i class="fas fa-user-shield"></i></a>
								<?php endif; ?>
							</td>
						<?php endif; ?>
					</tr>
