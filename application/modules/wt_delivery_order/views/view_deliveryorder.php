                  ...
                  foreach(@$getitemso as $kdo => $dt_spk){
                      ...
                      $cust    = $this->db->query("SELECT * FROM master_customers WHERE id_customer ='$id_cust->id_customer' ")->row();
						...
						<tr id='tr_$i$loop'>
							...
							<th > <input type='text' class='form-control' 		value='$dt_spk->metode_kirim'	 id='used_serial_number_$i$loop' required name='dt[$i$loop][serial_number]' readonly></th>
							<th > <input type='text' class='form-control' 		value='$dt_spk->keterangan_kirim'	 id='used_kartu_garansi_$i$loop' required name='dt[$i$loop][kartu_garansi]' readonly></th>
