...
class Wt_penawaran extends Admin_Controller
{
		...
		<tr id='tr_$loop'>
			...
			<td>
				<select id='used_no_surat_$loop' name='dt[$loop][no_surat]' data-no='$loop' onchange='CariDetail($loop)' class='form-control select' required>
					...
					echo"<option value='$produk->id_category3'>$produk->nama|$produk->kode_barang</option>";
					}
		echo	"</select>
			</td>
			...
			<td id='qty_$loop'><input type='text' class='form-control input-sm' id='used_qty_$loop' required name='dt[$loop][qty]'></td>
			...
			<td id='stok_tersedia_$loop'><input type='text' class='form-control input-sm' id='used_stok_tersedia_$loop' required name='dt[$loop][stok_tersedia]'></td>
			<td id='potensial_loss_$loop'><input type='text' class='form-control input-sm' id='used_potensial_loss_$loop' required name='dt[$loop][potensial_loss]'></td>
			<td id='diskon_$loop'><input type='text' class='form-control'  id='used_diskon_$loop' required name='dt[$loop][diskon]'></td>
			<td id='freight_cost_$loop'><input type='text' class='form-control input-sm' id='used_freight_cost_$loop' required name='dt[$loop][freight_cost]'></td>
			<td id='total_harga_$loop'><input type='text' class='form-control input-sm' id='used_total_harga_$loop' required name='dt[$loop][total_harga]' readonly></td>
			<td align='center'>
    ...
    public function SaveNewPenawaran()
    {
		...
		$data = [
                            ...
                            'nilai_penawaran'		=> $post['totalproduk'],
               ...
               foreach($_POST['dt'] as $used){
                   if(!empty($used[no_surat])){
                       ...
                       $dt[] =  array(
                               ...
                               'qty'			    => $used[qty],
                               'harga_satuan'		=> $used[harga_satuan],
                               'stok_tersedia'		=> $used[stok_tersedia],
                               'potensial_loss'		=> $used[potensial_loss],
                               'diskon'		        => $used[diskon],
                               'freight_cost'		=> $used[freight_cost],
                               'total_harga'	    => $used[total_harga],
                               'created_on'			=> date('Y-m-d H:i:s'),
                               'created_by'			=> $this->auth->user_id()                       
                               );
                   }
	...
	public function SaveRevisiPenawaran()
    {
		...
		approved_on,
		revisi_by,
		revisi_on')->where('no_penawaran',$code)->get('tr_penawaran');
		if($select1->num_rows())
		{
